...
function DefenseScreen({
  ...
  const [gameState, setGameState] = useState(() =>
    createInitialGameState(DEFAULT_GAME_MAP_ID),
  );
  const [selectedBuildTower, setSelectedBuildTower] =
    useState<TowerTypeId>("pulse");
  const [selectedPlacedTowerId, setSelectedPlacedTowerId] = useState<
    string | null
  >(null);
  ...
  const waves = useMemo(
    () => (isClassicMode ? activeLevel.waves : []),
    [activeLevel.waves, isClassicMode],
  );
  const activeMap = useMemo(
    () => loadGameMap(gameState.mapId),
    [gameState.mapId],
  );
  const currentWaveDefinition = useMemo(
    () =>
      isClassicMode
        ? (waves[gameState.waveIndex] ?? null)
        : buildEndlessWaveDefinition(gameState.waveIndex),
  ...
  const cellSize = Math.max(
    14,
    Math.min(boardMaxWidth / activeMap.cols, boardMaxHeight / activeMap.rows),
  );
  ...
  useEffect(() => {
    ...
    const stillExists = gameState.towers.some(
      (tower) => tower.id === selectedPlacedTowerId,
    );
    if (!stillExists) {
  ...
  const statusText = useMemo(() => {
    if (gameState.status === "won") {
      return isClassicMode
        ? "Classic level cleared."
        : "Endless surge repelled.";
    }
  ...
  const isCountdownActive =
    hasStarted && gameState.status === "running" && countdownSeconds > 0;
  ...
  const selectedPlacedTower = useMemo(
    () =>
      gameState.towers.find((tower) => tower.id === selectedPlacedTowerId) ??
      null,
    [gameState.towers, selectedPlacedTowerId],
  );

  const selectedPlacedTowerStats = selectedPlacedTower
    ? getTowerStats(selectedPlacedTower)
    : null;
  const selectedUpgradeCost = selectedPlacedTower
    ? getTowerUpgradeCost(selectedPlacedTower)
    : null;
  const selectedSellValue = selectedPlacedTower
    ? getTowerSellValue(selectedPlacedTower)
    : null;
  ...
  const canUpgradeSelectedTower = selectedPlacedTower
    ...
    : false;
  const showClassicCompletionModal =
  ...
  const handleBoardPress = (event: GestureResponderEvent) => {
    ...
    const tappedTower = gameState.towers.find(
      (tower) => tower.cell.col === col && tower.cell.row === row,
    );
    if (tappedTower) {
      setSelectedPlacedTowerId((previousId) =>
        previousId === tappedTower.id ? null : tappedTower.id,
      );
      return;
    }
  ...
  const handleUpgradeTower = () => {
    ...
    setGameState((previousState) =>
      upgradeTower(previousState, selectedPlacedTower.id),
    );
  };
  ...
  const handleSellTower = () => {
    ...
    setGameState((previousState) =>
      sellTower(previousState, selectedPlacedTower.id),
    );
    setSelectedPlacedTowerId(null);
  };
  ...
  const handleCycleTargetMode = () => {
    ...
    setGameState((previousState) =>
      cycleTowerTargetMode(previousState, selectedPlacedTower.id),
    );
  };
  ...
  const handleSwitchMap = (mapId: GameMapId) => {
    ...
    setGameState(
      createInitialGameState(mapId, nextLevelId, gameState.gameMode),
    );
    setSelectedPlacedTowerId(null);
    setHasStarted(false);
    setIsPaused(true);
      ...
      <View style={styles.gameShell}>
        <View style={styles.boardPane}>
          <View style={styles.topBar}>
            ...
            <View style={styles.topBarRight}>
              <Pressable
                onPress={() =>
                  setSimulationSpeed((value) => (value === 2 ? 1 : 2))
                }
                style={[
                  styles.quickButton,
                  simulationSpeed === 2 && styles.quickButtonActive,
                ]}
              >
              ...
              <Pressable
                ...
                style={[
                  styles.quickButton,
                  isMenuOpen && styles.quickButtonActive,
                ]}
              >
          ...
          <View style={styles.boardFrame}>
            ...
            >
              ...
              {selectedPlacedTower && selectedTowerBadgePosition ? (
                ...
                >
                  <Text style={styles.towerLevel}>
                    L{selectedPlacedTower.level}
                  </Text>
                </View>
              ) : null}
            </Pressable>
          ...
          {isMenuOpen ? (
            <View style={styles.menuPanel}>
              ...
              <View style={styles.menuModeRow}>
                <Pressable
                  style={[styles.menuModeButton, styles.menuModeButtonActive]}
                >
                ...
                {(["classic", "endless"] as GameMode[]).map((mode) => {
                  ...
                  return (
                    <Pressable
                      ...
                      style={[
                        styles.menuModeButton,
                        isActive && styles.menuModeButtonActive,
                      ]}
                    >
                      <Text style={styles.menuModeButtonText}>
                ...
                {gameMaps.map((map) => {
                  ...
                  return (
                    <Pressable
                      ...
                      style={[
                        styles.menuMapButton,
                        isActive && styles.menuMapButtonActive,
                      ]}
                    >
              ...
              {!isClassicMode ? (
                <Text style={styles.menuHintText}>
                  Endless runs indefinitely. Every 10th wave is a challenge
                  surge.
                </Text>
              ) : null}
              ...
              <View style={styles.menuActions}>
                <Pressable
                  ...
                  style={[styles.menuActionButton, styles.menuRestartButton]}
                >
        ...
        <View style={[styles.sidePanel, { width: sidePanelWidth }]}>
          ...
          <View style={styles.detailsShell}>
            ...
            selectedSellValue !== null ? (
              <View style={styles.selectedPanel}>
                <Text style={styles.selectedPanelTitle}>
                  {TOWER_TYPES[selectedPlacedTower.towerType].label}{" "}
                  {selectedPlacedTower.id} Lv {selectedPlacedTower.level}
                </Text>

                <Text style={styles.selectedPanelStats}>
                  Dmg {selectedPlacedTowerStats.damage} · Range{" "}
                  {selectedPlacedTowerStats.range.toFixed(1)} · RoF{" "}
                ...
                <View style={styles.selectedPanelActions}>
                  <Pressable
                    ...
                    style={[
                      styles.panelButton,
                      !canUpgradeSelectedTower && styles.panelButtonDisabled,
                    ]}
              ...
              <Text style={styles.helperText}>
                Tap grid to place {selectedTowerType.label} ($
                {selectedTowerType.cost}). Tap placed towers to manage.
              </Text>
            )}
          </View>
        </View>
